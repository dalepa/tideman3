...
String Version = "Tideman OTA V5.4 - AF-ESP32-S2";                       // Version 
String BoardId = "Tideman3.ktxolivi2-800";         //ESP32 - Victron MPPT 100/30 
...
void logall(int loginterval)    //loginterval in seconds
{

      // log other data  here
      
      int elapsed = (millis() - lastLogTime) / 1000;
      ...
      if (elapsed > loginterval)
      {
        ...
        Serial.println("logging for seconds: " + String(lastLogTime/1000));
...
void deepSleeplog(int seconds, int sleeptime)
{

  sleeptime = sleeptime*1000000;
  

  if ( maxlipo.cellPercent() < 50)  //start sleeping at 50%
  {
    while (millis() < seconds * 1000)  //log for at least seconds afer deep sleeping
    {
      
      getDistance(); // get distance ASAP
      logall(5);

      // webserver.handleClient();  //dont allow webserver clients at 50%
    }
  ...
  getDistance();
  logall(5);  //log for seconds .  wont get called if deep sleeping
